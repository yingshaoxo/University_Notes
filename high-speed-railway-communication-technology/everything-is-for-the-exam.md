...
# 判断题
* 音源板提供对数字模块层的驱动电路（ ）
* 音源板为系统提供各种回铃音和忙音等提示音（ ）110P
* 列车调度电话的电路是独立封闭型的 （ ）
* 列车调度电话的电路是共线连接（ ）
* 四线接口是供接入列车无线调度电话（ ）184-185P
* 数字调度主机通过2B+D接口与操作台连接（ ）
* 共电接口用以接入站场扩音、广播设备（ ）
* 磁石接口接人磁石电话用户（ ）
...
* 站场通信主要通过车站分系统实现 （ ）
* U口板提供2B＋D接口（ ）
* FH98调度主系统配接最大E1数字接口数是24（ ）
* FH98调度主系统最大配接调度台数是48 （ ）
* FH98调度分系统模拟接口数是138（ ）
* FH98调度分系统E1数字接口是4（ ）
* FH98调度分系统2B＋D接口是46（ ）
* FH98调度分系统最多配接值班台数是15（ ）
* 应急分机可替代值班台实现通信业务（ ）
* FH98调度主系统允许一个数字环上挂接的车站最多50个 （ ）
* FH98调度系统共分接口可与站场广播系统提供的共电接口对接 （ ）
* FH98调度主系统模拟接口是180（ ）
* FH98调度主系统2B＋D接口是60（ ）
* 电源模块实际上是二次电源（ ）
* 资源模块提供系统所需的各种公共资源（ ）
* CTT4000用以实现高铁调度通信（ ）194P
* 性能管理可显示系统的网络拓扑结构（ ）203-204P
* 配置管理可以进行网络通道的配置（ ）
* 故障管理可进行系统所有告警、故障信息的收集 （ ）
* FH98调度主系统最多可接入数字环数是12（ ）
* MDS3400系统用于多媒体调度通信（ ）
